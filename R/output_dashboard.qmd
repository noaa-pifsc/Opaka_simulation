...
#save ts_re to report folder so it can be used to make figures directly
#write.csv(ts_re, file = "../Report/ts_re.csv", row.names = FALSE)
```
...
facet_grid(Rec_scenario ~ scen_label, scales = "free_y") + 
...
ggplot(aes(x = year)) + 
geom_point(aes(y = Obs, color = scen_label)) +
geom_line(aes(y = Exp, group = scen_label, color = scen_label)) +
...
geom_hline(yintercept = 0, linetype = 2, color = "grey50") +
geom_boxplot(aes(x = year, y = residuals, group = year)) + 
geom_smooth(aes(group = scen_label, color = scen_label)) +
facet_grid(scen_label ~ Rec_scenario) + 
ylim(-.5, .5) +
...
ggplot(aes(x = Bin)) + 
geom_ribbon(data = . %>% filter(model_run == "em"), aes(ymin = 0, ymax = Obs, fill = scen_label), alpha = .35) +

geom_line(data = . %>% filter(model_run == "em"), aes(y = Exp, color = scen_label)) +
facet_wrap(~year) + 
...
geom_line(data = . %>% group_by(scen_label, Rec_scenario, Yr) %>% summarise(med_exp = quantile(Expmn, .5)), aes(y = med_exp, color = scen_label), linewidth = 1.5) + 
facet_grid(scen_label ~ Rec_scenario) +
...
ts_re %>%
  ...
  summarise(
    med_re = quantile(SpawnBio_re, 0.5, na.rm = TRUE),
    uci = quantile(SpawnBio_re, 0.975, na.rm = TRUE),
    lci = quantile(SpawnBio_re, 0.05, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  extract(
    scenario,
  ...
  geom_ribbon(
    aes(ymin = lci, ymax = uci, fill = scen_label),
    alpha = 0.25,
    color = NA
  ) +
  geom_line(aes(y = med_re, color = scen_label), linewidth = 1.2) +
  facet_wrap(~Rec_scenario) +
  labs(x = "Year", y = "SSB MRE", color = "Scenario", fill = "Scenario") +
  theme_bw()
```
...
ggplot() +
geom_hline(yintercept = 0, color = "grey60") +
geom_line(aes(x = year, y = SpawnBio_om, color = scen_label), linetype = 2) + 
geom_line(aes(x = year, y = SpawnBio_em, color = scen_label)) +
facet_grid(scen_label ~ Rec_scenario) + 
labs(x = "Year", y = "SSB") + 
theme_bw()
```
...
ggplot() +
geom_hline(yintercept = 0, color = "grey60") +
geom_ribbon(aes(x = year, ymin = lci, ymax = uci, fill = scen_label), alpha = .25) + 
geom_line(aes(x = year, y = med_re, color = scen_label), linewidth = 1.2) +
facet_wrap(~Rec_scenario) + 
labs(x = "Year", y = "F MRE") + 
theme_bw()
...
ggplot() +
geom_hline(yintercept = 0, color = "grey60") +
geom_line(aes(x = year, y = F_1_om, color = scen_label), linetype = 2) + 
geom_line(aes(x = year, y = F_1_em, color = scen_label)) +
facet_grid(scen_label ~ Rec_scenario) + 
labs(x = "Year", y = "F") + 
theme_bw()
```
...
ggplot() +
geom_boxplot(aes(x = scen_label, y = value, fill = scen_label)) + 
geom_hline(yintercept = 0, color = "grey50", linetype = 2) +
facet_wrap(~name, scales = "free_y") +
scale_x_discrete(labels = NULL, breaks = NULL) +
